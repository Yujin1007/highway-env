...
class CustomContinuousAction(ActionType):
    ...
    ANGULAR_V_RANGE = (-np.pi / 9, np.pi / 9)
    ...
    def __init__(self,
        ...
        self.acceleration_range = acceleration_range if acceleration_range else self.ACCELERATION_RANGE
        self.steering_range = steering_range if steering_range else self.STEERING_RANGE
        self.angular_v_range = self.ANGULAR_V_RANGE
        self.lateral = lateral
        self.longitudinal = longitudinal
        if not self.lateral and not self.longitudinal:
            raise ValueError("Either longitudinal and/or lateral control must be enabled")
    ...
    def act(self, action: np.ndarray) -> None:
        if self.clip:
            action = np.clip(action, -1, 1)
        if self.longitudinal and self.lateral:
            omega = utils.lmap(action[1], [-1, 1], self.angular_v_range)

            steer = np.clip(self.env.steer + omega * self.dt, self.steering_range[0], self.steering_range[1])
            ...
            # print(self.controlled_vehicle.action)
...
def action_factory(env: 'AbstractEnv', config: dict) -> ActionType:
    ...
    elif config["type"] == "CustomContinuousAction":
        return CustomContinuousAction(env, **config)
    else:
