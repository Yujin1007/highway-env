...
class TD3(OffPolicyAlgorithm):
    ...
    def train(self, gradient_steps: int, batch_size: int = 100) -> None:
        ...
        for _ in range(gradient_steps):
            ...
            replay_data = self.replay_buffer.sample(batch_size, env=self._vec_normalize_env) # decision buffer
            # replay_data = self.replay_buffer.sample(batch_size, env=self._vec_normalize_env) # normal buffer 
            
            decision_replay = replay_data.observations['decision'][:,0]
            cnt_idle = np.where(decision_replay == 1)[0].shape[0] / batch_size
            cnt_change = 1 - cnt_idle
        ...
        self.logger.record('train/buffer', self.replay_buffer.pos)
